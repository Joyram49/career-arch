...
const envSchema = z
  .object({
    ...
    // ── Email (Brevo): use BREVO_API_KEY on hosts that block SMTP (e.g. Render free tier) ──
  ...
  .superRefine((data, ctx) => {
    const hasApi = typeof data.BREVO_API_KEY === 'string' && data.BREVO_API_KEY.trim().length > 0;
    const hasSmtpPair =
      typeof data.BREVO_SMTP_USER === 'string' &&
      data.BREVO_SMTP_USER.trim().length > 0 &&
      typeof data.BREVO_SMTP_KEY === 'string' &&
      data.BREVO_SMTP_KEY.trim().length > 0;

    if (hasApi || hasSmtpPair) return;

    ctx.addIssue({
      code: z.ZodIssueCode.custom,
      message: 'Provide BREVO_API_KEY or both BREVO_SMTP_USER and BREVO_SMTP_KEY',
      path: ['BREVO_API_KEY'],
    });
  });
